...
void main() {
  ...
  group('Memory Pressure Integration', () {
    ...
    tearDown(() async {
      ...
      // But handler.dispose() removes observer from WidgetsBinding,
      //avoiding leak?
      // Since it's a singleton, we might affect other tests if we
      // dispose it fully.
    ...
    test('Trims cache critically on memory pressure', () {
      fakeAsync((async) {
        ...
        cache
          ...
          // Let's use `QueryClient` or `Query` to create a real subscription
          //if possible,
          ...
          // Problem: I can't easily set refCount > 0 without using high level
          //APIs like QueryObserver.
        ...
        handler.simulateMemoryPressure();
        ...
        expect(cache.get<String>('fresh'), isNull);
      });
    });
