...
export const DEFAULT_DESCRIPTION =
  "Fasq is a resilient, security-first async state management solution for Flutter. Features hardware-backed encryption, zero-boilerplate serialization, and high-performance caching for Bloc, Riverpod, and Hooks.";
...
export const buildDocMetadata = ({
  ...
  const titleFromPath =
    mdxPath && mdxPath.length > 0 ? toStartCase(mdxPath) : "Documentation";
  ...
  const twitterCard = hasTwitterCard(mdxTwitter)
    ? mdxTwitter.card
    : "summary_large_image";
