...
class GlobalQueryEffects extends QueryClientObserver {
  ...
  static GlobalQueryEffects install({
    ...
    required EffectMessenger messenger,
  ...
  void onMutationSuccess(
    ...
    final message = meta?.successMessage;
    if (message != null) {
      final resolved = resolveMessage?.call(message) ?? message;
      final state = context != null && context.mounted
          ? ScaffoldMessenger.maybeOf(context)
          : messenger.key.currentState;

      state?.showSnackBar(SnackBar(content: Text(resolved)));
    }
...
`EffectMessenger` is a lightweight helper that wraps a `GlobalKey<ScaffoldMessengerState>`; feel free to implement your own messenger to route toast/snackbar delivery.

`MutationBuilder` and `QueryBuilder` automatically capture the current `BuildContext` and forward it to observers. When mutations run outside the widget tree, the observer still fires with `context == null`, so global services (scaffold messenger, navigator, auth) can take over.

`EffectMessenger` is optional—when omitted, observers will use the forwarded `BuildContext` and silently skip UI work if no messenger or context is available.
        ...
        children: [
          ElevatedButton(
            onPressed: () {
              ...
              queryClient.setQueryData('users'.toQueryKey(), [
                User(id: '1', name: 'John'),
                User(id: '2', name: 'Jane'),
              ]);
          ...
          ElevatedButton(
            onPressed: () {
              ...
              // Get query data
              final users = queryClient.getQueryData<List<User>>('users'.toQueryKey());
              if (users != null) {
          ...
          ElevatedButton(
            onPressed: () {
              ...
              queryClient.removeQuery('users'.toQueryKey());
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              final query = queryClient.getQueryByKey<List<User>>('users'.toQueryKey());
              if (query != null) {
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              final exists = queryClient.hasQuery('users'.toQueryKey());
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              queryClient.prefetchQuery('users'.toQueryKey(), () => api.fetchUsers());
              queryClient.prefetchQuery('posts'.toQueryKey(), () => api.fetchPosts());
              queryClient.prefetchQuery('comments'.toQueryKey(), () => api.fetchComments());
        ...
        children: [
          ElevatedButton(
            onPressed: () {
              ...
              queryClient.setQueryData('temp:data'.toQueryKey(), {'temp': true});
