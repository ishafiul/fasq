...
QueryBuilder<List<Todo>>(
  ...
  queryFnWithToken: (token) async {
    // Pass the token to your HTTP client (e.g., Dio)
    // You might need an extension to convert CancellationToken to your client's format,
    // or check token.isCancelled manually.
    final response = await dio.get(
      '/todos',
      cancelToken: token.asDioCancelToken(), // Hypothetical extension
    );
